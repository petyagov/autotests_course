...
class Trigon:
    """
    Класс для проверки на корректность переданных данных или генерируются исключения
    """
    def __init__(self, *args):
        """
        Функция принимает неизвестное количество атрибутов и выдаёт исключения по результату проверки
        :param args: неизвестное кол-во атрибутов
        """
        for i in args:
            if type(i) != int:
                raise TypeError('Стороны должны быть числами')  # Поднимаем исключение
            elif i <= 0:
                raise ValueError('Стороны должны быть положительными')  # Поднимаем исключение
        if len(args) < 3:
            raise IndexError(f'Передано {len(args)} аргументов, а ожидается 3')  # Поднимаем исключение
        elif args[0] + args[1] < args[2] or args[1] + args[2] < args[0] or args[2] + args[0] < args[1]:
            raise Exception('Не треугольник')  # Поднимаем исключение
