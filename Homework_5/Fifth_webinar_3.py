...
def everything_for_your_cat(cats_data):
    """
    Функция выводит список с владельцами и кошками в определённом формате
    :param cats_data: список кортежей с данными о кошках и их владельцах
    :return: список с владельцами и кошками
    """
    our_str = ""
    dict = {}
    for host in cats_data:
        if f'{host[2]} {host[3]}:' not in dict:
            dict[f'{host[2]} {host[3]}:'] = f'{host[0]}, {host[1]}'
        else:
            dict[f'{host[2]} {host[3]}:'] = dict.get(f'{host[2]} {host[3]}:') \
                                            + f'; {host[0]}, {host[1]}'
    for men, pet in dict.items():
        our_str += f'{men} {pet}\n'
    return our_str
