...
def repeats(our_str):
    """
    Получаем строку, где отображается каждый символ и количество раз сколько он уже встречался в строке
    :param our_str: строка, из которой берём символы и количество повторений
    :return: строка, которую получаем
    """
    # При помощи генератора создаем строку, используя условие и форматирование строки
    # В первом условии создаём строку, при помощи форматирования подставляем бегунок а
    # и количество раз, которое уже встречался в строке (вычисляется методом count())
    # иначе только а_1
    new_str = ''
    for i in our_str:
        if i in new_str:
